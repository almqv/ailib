...
class neural_network:
    ...
    def generateLayers(self, neuronDimensions: list = [1, 1], weightMin: float = 0.0, weightMax: float = 1.0, biasMin: float = -2.0, biasMax: float = 2.0):
        ...
        try:
            ...
            for index, neuronCount in enumerate(neuronDimensions):
                if(index > 0):
                    self.weights[index - 1] = np.random.default_rng().uniform(
                        weightMin, weightMax, [neuronDimensions[index-1], neuronCount])
            ...
            for index, neuronCount in enumerate(neuronDimensions):
                if(index > 0):
                    self.bias[index - 1] = np.random.default_rng().uniform(biasMin,
                                                                           biasMax, [1, neuronCount])
    ...
    def teachSGD(self, theta: float = 0.001, lr: float = 0.1):
        try:
            gen = 0  # the generation
            inp = None  # input, gets randomized each generation

            while(gen <= self.teachTimes):
                # Use the networks data feeder function pointer to pick random inputs
                inp = self.dataFeeder(gen, self.inputDimensions)
            ...
            self.debug(
                f"[{self.teachTimes}/{self.teachTimes}] Teaching finished! Error: {meanErr}", db.level.success, end="\r\n")

        except:
            if(self.correctFuncPointer == None or self.dataFeederFuncPointer == None):
                self.debug(
                    "Invalid or unassigned function pointers. Network will not be able to learn.", db.level.fail)
